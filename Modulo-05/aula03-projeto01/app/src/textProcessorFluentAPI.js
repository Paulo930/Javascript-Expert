...
class TextProcessorFluentAPI {
  ...
  extractPeopleData() {
    ...
    // [contratante|contratada] ou um ou outro, (e tem a flag no fim da expressão pra maiusculo e minusculo)
    // :\s{1} vai procurar o caracter literal do dois pontos seguindo de espaço
    ...
    // faz a match para encontrar a string inteira que contem os dados que precisamos
    const onlyPerson = this.#content.match(matchPerson);
    // console.log('onlyPerson', matchPerson.test(matchPerson));
  ...
  removeEmptyCharacters() {
    ...
    this.#content = this.#content.map((line) =>
      line.map((item) => item.replace(trimSpace, '')),
    );
    return this;
  }
